...
void insert(heap_t *heap, tree_t *data){
    heap->last_index += 1;
...
void Encoder(heap_t *h){
    tree_t *t1 = DeleteMin(h);
    tree_t *t2 = DeleteMin(h);
...
tree_t *printCode(tree_t *t, Stack **s, char *word, char d) {
    if (t != NULL) {
        if (t->text != NULL && strcmp(t->text, word) == 0) {
            if (d == 'l') {
                (*s)->data = '0';
            } else {
                (*s)->data = '1';
...
int main(){
    ...
    //check while encoder doesn't finish 
