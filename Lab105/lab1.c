...
datetime_t createDate(int timeAmount) {
    datetime_t inputDate;
    inputDate.second = timeAmount % 60;
    timeAmount /= 60;
    inputDate.minute = timeAmount % 60;
    timeAmount /= 60;
    inputDate.hour = timeAmount % 24;
    timeAmount /= 24;
    inputDate.dayOfWeek = timeAmount % 7;
    inputDate.month = 1;
    inputDate.year = 1970;
    
    while (timeAmount > dayOfMonth(inputDate.month, inputDate.year)) {
        timeAmount -= dayOfMonth(inputDate.month++, inputDate.year);
        if (inputDate.month > 12 && inputDate.year++) {
            inputDate.month = 1;
        }
    }
    
    if (timeAmount + 1 > dayOfMonth(inputDate.month, inputDate.year)) {
        timeAmount = 0;
        inputDate.month++;
    }
    
    inputDate.date = ++timeAmount;
    return inputDate;
}
...
int main(void) {
    ...
    int timeAmount;
    scanf("%d", &timeAmount);
    inputDate = createDate(timeAmount);
    printDate(inputDate);
    return 0;
}
